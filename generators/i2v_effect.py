...
class WanI2VEffectGenerator(WanAPIBase):
    """Node for image-to-video generation with effects using Wan model"""
    
    # Define available Wan i2v models
    MODEL_OPTIONS = [
        "wan2.1-i2v-plus"    # Professional Edition
    ]
        ...
        return {
            "required": {
                "model": (cls.MODEL_OPTIONS, {
                    "default": "wan2.1-i2v-plus"
                }),
                "image_url": ("STRING", {
                    "default": "https://example.com/your_image.png"
                }),
                "template": (cls.TEMPLATE_OPTIONS, {
                    "default": "flying"
                }),
                "region": (cls.REGION_OPTIONS, {
                    "default": "mainland_china"  # Note: Effects are only supported in Mainland China region
                })
            },
            "optional": {
    ...
    def generate(self, model, image_url, template, region, resolution="720P", 
                 seed=0, output_dir="./videos"):
        ...
        # Note: For effects, the prompt is ignored, so we don't include it
        # Prepare API payload for image-to-video generation with effects
        payload = {
            "model": model,
            "input": {
                "img_url": image_url,
    ...
    def poll_task_result(self, task_id, output_dir="./videos", region="mainland_china"):
            ...
            try:
                ...
                if task_status == "SUCCEEDED":
                    ...
                    if "video_url" in result["output"]:
                        ...
                        video_filename = f"wan_i2v_effect_{timestamp}.mp4"
